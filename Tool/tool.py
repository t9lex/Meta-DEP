...
loss_stats = 10000
for epoch in range(1, 5000): #7000
    loss = train()
    # print(f'Epoch: {epoch:02d}, Loss: {loss:.4f}')
    if loss < loss_stats:
        loss_stats = loss
        torch.save(model.state_dict(), './model/model_state/metapath2vec.pt')
    # scheduler.step()
model.load_state_dict(torch.load('./model/model_state/metapath2vec.pt'))
model.eval()    
...
print("############## monomer_screen #################")
print(danti_list)
print("Number of monomer available: ",len(danti_list))
